...
@interface TBDemo1 ()<TBSrollViewEmptyDelegate>
...
- (NSAttributedString *)tb_emptyTitle:(UIScrollView *)scrollView network:(TBNetworkStatus)status {
    ...
    if (status == TBNetworkStatusNotReachable) {
        NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:@"网络异常" attributes:@{NSForegroundColorAttributeName : [UIColor redColor]}];
        return attrString;
    } else {
        NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:@"网络正常" attributes:@{NSForegroundColorAttributeName : [UIColor greenColor]}];
        return attrString;
    }
}
